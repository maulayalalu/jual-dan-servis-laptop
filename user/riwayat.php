...
<div style="min-height:100vh;background:var(--color-light-ash);padding-top:72px;">
  <div class="container" style="max-width:1100px;padding-top:32px;padding-bottom:80px;">
    ...
    <!-- Tabs -->
    <div style="display:flex;gap:12px;border-bottom:1px solid var(--color-cloud);margin-bottom:24px;">
        <a href="?tab=transaksi" style="padding:12px 16px;font-weight:500;font-size:15px;color:<?= $activeTab==='transaksi' ? 'var(--color-blue)' : 'var(--color-pewter)' ?>;border-bottom:3px solid <?= $activeTab==='transaksi' ? 'var(--color-blue)' : 'transparent' ?>;transition:all 0.3s;">
            Pesanan Produk
        </a>
        <a href="?tab=servis" style="padding:12px 16px;font-weight:500;font-size:15px;color:<?= $activeTab==='servis' ? 'var(--color-blue)' : 'var(--color-pewter)' ?>;border-bottom:3px solid <?= $activeTab==='servis' ? 'var(--color-blue)' : 'transparent' ?>;transition:all 0.3s;">
            Layanan Servis
        </a>
    </div>
        ...
        <div class="table-wrap">
            <table>
                ...
                <?php if ($riwayatTransaksi->num_rows > 0): while ($t = $riwayatTransaksi->fetch_assoc()): ?>
                    <tr>
                        <td><code style="font-size:12px;color:var(--color-pewter);background:var(--color-light-ash);padding:2px 6px;border-radius:4px;"><?= htmlspecialchars($t['order_id']) ?></code></td>
                        ...
                        <td>
                            ...
                            <?php elseif ($t['status_pembayaran'] === 'pending_verify'): ?>
                                <span style="font-size:12px;color:var(--color-pewter);">⏳ Sedang diverifikasi admin</span>
                            <?php endif; ?>
            ...
            <table>
                ...
                <?php if ($riwayatServis->num_rows > 0): while ($s = $riwayatServis->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td style="color:var(--color-blue);font-weight:500;"><?= $s['biaya'] > 0 ? formatRupiah($s['biaya']) : '<span style="color:var(--color-silver-fog);font-weight:400;">Menunggu Pengecekan</span>' ?></td>
