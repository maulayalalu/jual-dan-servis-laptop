    ...
    <div style="display:grid;grid-template-columns:1fr 280px;gap:24px;align-items:start;">
      <div>
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
          <div style="width:40px;height:40px;border-radius:50%;background:var(--color-blue);color:white;display:grid;place-items:center;font-weight:600;"><?= strtoupper(substr($t['nama'],0,1)) ?></div>
          <div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:14px;background:var(--color-light-ash);border-radius:8px;padding:16px;">
          <div><span style="color:var(--color-pewter);">Order ID:</span><br><code style="font-size:12px;"><?= htmlspecialchars($t['order_id']) ?></code></div>
          <div><span style="color:var(--color-pewter);">Total:</span><br><span style="font-weight:600;color:var(--color-blue);"><?= formatRupiah($t['total_harga']) ?></span></div>
        ...
        <div style="display:flex;gap:8px;margin-top:16px;">
          <form method="POST" style="display:inline;">
            ...
            <button type="submit" class="btn btn--primary" onclick="return confirm('Konfirmasi pembayaran ini sebagai LUNAS?');">✓ Terima — Tandai Lunas</button>
          </form>
          <form method="POST" style="display:inline;">
            <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
            <input type="hidden" name="action" value="reject">
            <input type="hidden" name="id_transaksi" value="<?= $t['id_transaksi'] ?>">
            <button type="submit" class="btn btn--danger" onclick="return confirm('Tolak bukti pembayaran ini?');">✗ Tolak</button>
          </form>
        </div>
      </div>
      <!-- Bukti Image -->
      <div>
        ...
        <a href="../<?= htmlspecialchars($t['bukti_pembayaran']) ?>" target="_blank">
          <img src="../<?= htmlspecialchars($t['bukti_pembayaran']) ?>" 
               alt="Bukti Transfer" 
               style="width:100%;border-radius:8px;border:1px solid var(--color-cloud);cursor:zoom-in;"
